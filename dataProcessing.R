...
#load all data
all <- nc_open("/Users/michaeldiamond/Documents/Data/CERES/SSF1deg/SSF1deg_shipkrige_Aqua.nc")
...
xCx <- which(lon >= -12.5 & lon <= 7.5); nXCx <- length(xCx)
yCx <- which(lat >= -18 & lat <= -8); nYCx <- length(yCx)
...
possBoxCx <- EDGAR_SO2_AnnAv[xCx[1]:xCx[nXCx], yCx[1]:yCx[nYCx]]
rowMaxCx <- apply(possBoxC, 2, which.max)
shipCx <- shipCxInd <-  matrix(ncol = 2, nrow = 0)
for (i in 1:nYCx) {
  l1 <- rowMaxC[i] - bdShip + xCx[1] - 1 - 4
  l2 <- rowMaxC[i] + bdShip + xCx[1] - 1
  shipCx <- rbind(shipCx, cbind(lon[l1:l2], rep(lat[yCx[i]], 5)))
  shipCxInd <- rbind(shipCxInd, cbind(l1:l2, rep(yCx[i], 5)))
}
...
CxBox <- matrix(nrow = 360, ncol = 180, data = FALSE)
CxBoxInd <- expand.grid(xCx, yCx)
CxBox[CxBoxInd[,1], CxBoxInd[,2]] <- TRUE

#Remove ship track from NE and SE box
NEBox[shipNEInd] <- FALSE
SEBox[shipSEInd] <- FALSE
CBox[shipCInd] <- FALSE
CxBox[shipCxInd] <- FALSE
...
notShipCxInd <- which(CxBox, arr.ind = T)
notShipCx <- cbind(lon[notShipCxInd[,1]], lat[notShipCxInd[,2]])
...
regInfo <- list("notShipNE" = notShipNE, "notShipSE" = notShipSE, 
                ...
                "notShipC" = notShipC, "notShipCx" = notShipCx,
                "shipNE" = shipNE, "shipSE" = shipSE,
                "shipNW" = shipNW, "shipSW" = shipSW, 
                "shipC" = shipC, "shipCx" = shipCx,
                "notShipNEInd" = notShipNEInd, "notShipSEInd" = notShipSEInd, 
                "notShipNWInd" = notShipNWInd, "notShipSWInd" = notShipSWInd, 
                "notShipCInd" = notShipCInd, "notShipCxInd" = notShipCxInd,
                "shipNEInd" = shipNEInd, "shipSEInd" = shipSEInd,
                "shipNWInd" = shipNWInd, "shipSWInd" = shipSWInd, 
                "shipCInd" = shipCInd, "shipCxInd" = shipCxInd)
save(regInfo, file = "/Users/michaeldiamond/Documents/Projects/ERB23_ShipTracks4MCB/ShipClosure/processed/regInfo.rda")
