...
analyze <- function(respName, regName, covars,
  ...
  pdf(sprintf("/Users/michaeldiamond/Documents/Projects/ERB23_ShipTracks4MCB/ShipClosure/Results/Figures_%s_%s_M%sto%s.pdf", respName, regName, months[1], months[length(months)]))
  ...
  if (regName == "NE") {
    ...
    notShip <- regInfo$notShipCx; ship <- regInfo$shipCx
    notShipInd <- regInfo$notShipCxInd; shipInd <- regInfo$shipCxInd
  } else if (regName == "SE") {
  ...
  if (savenc) {
    ...
    shipdim <- ncdim_def("ship_dim","NA",c(1:length(respMean[shipInd])))
    ...
    ncfname <- sprintf("/Users/michaeldiamond/Documents/Projects/ERB23_ShipTracks4MCB/ShipClosure/Results/Data_%s_%s_M%sto%s.nc", respName, regName, months[1], months[length(months)])
